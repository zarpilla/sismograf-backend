...
module.exports = {
  ...
  async getWheelImageWithDescription(data, ctx, texts) {
      ...
      chartSummary.domains.forEach((d, i) => {
        d.principles.forEach((p, j) => {
          principles.push({
            label: `p${3 * i + j}v`,
            description: `${j + 1}. ${p.principleName}`,
            value: 3 * i + j === 2 ? 17 : toPct(p.resilienceLevel),
          });
          principles.push({
            label: `p${3 * i + j}`,
            value: 3 * i + j === 2 ? 100 - 17 : 100 - toPct(p.resilienceLevel),
          });
          c++;
        });
      });
    ...
    svg.attr(
      "transform",
      "translate(" + radius + "," + height / 2 + ") rotate(-45)"
    );
    ...
    slice
      ...
      .attr("points", function (d) {
        var posA = arc0.centroid(d); // line insertion in the slice
        var posB = arc6.centroid(d); // line break: we use the other arc generator that has been built only for that
        var posC = arc6.centroid(d); // Label position = almost the same as posB
        var midangle = d.startAngle + (d.endAngle - d.startAngle) / 2; // we need the angle to see if the X position will be at the extreme right or extreme left
        posC[0] = radius * 0.94 * (midangle < Math.PI ? 1 : -1); // multiply by 1 or -1 to put it on the right or on the left
    ...
    slice5
      ...
      .attr("points", function (d) {
        var posA = arc4.centroid(d); // line insertion in the slice
        var posB = arc6.centroid(d); // line break: we use the other arc generator that has been built only for that
        var posC = arc6.centroid(d); // Label position = almost the same as posB
        var midangle = d.startAngle + (d.endAngle - d.startAngle) / 2; // we need the angle to see if the X position will be at the extreme right or extreme left
        posC[0] = radius * 0.94 * (midangle < Math.PI ? 1 : -1); // multiply by 1 or -1 to put it on the right or on the left
    ...
    slice7
      ...
      .attr("points", function (d) {
        var posA = arc5.centroid(d); // line insertion in the slice
        var posB = arc6.centroid(d); // line break: we use the other arc generator that has been built only for that
        var posC = arc6.centroid(d); // Label position = almost the same as posB
        var midangle = d.startAngle + (d.endAngle - d.startAngle) / 2; // we need the angle to see if the X position will be at the extreme right or extreme left
        posC[0] = radius * 0.94 * (midangle < Math.PI ? 1 : -1); // multiply by 1 or -1 to put it on the right or on the left
    ...
    var textglobal = svg
      .select(".global")
      .selectAll("text")
      .data(pie(globalValue(texts, ctx)), key);
