...
const calculateResilienceLevel = async (data) => {
  ...
  try {
    await strapi
      .query("api::analysis.analysis")
      .update({ where: { id: data.id }, data: { _internal: false } });
  } catch (e) {
    console.error(e);
    await strapi.query("api::analysis.analysis").update({
      where: { id: data.id },
      data: { resilienceLevel: 0, _internal: false },
    });
  }
};
...
module.exports = {
  ...
  async afterUpdate(event) {
    ...
    if (event.params.data._internal) {
      return;
    }
    try {
      ...
      analysis.results.forEach((r) => {
        if (r.indicator) {
          ...
          r.domainDescription =
            r.indicator.pattern.principle.domain.description;
      ...
      await strapi.query("api::analysis.analysis").update({
        where: { id: id },
        data: { resilienceLevel: resilienceLevel, _internal: true },
      });
    } catch (e) {
      console.error(e);
    }
